...
import { hashQuerySet, activeQuerySet } from "../src/lib/query-set";
...
async function main() {
  ...
  const computed = await hashQuerySet(await activeQuerySet({ DB } as never, slug));
  const logged: any = runSql(
    `SELECT set_hash, question_count, datetime(observed_at,'unixepoch') at
       FROM query_set_versions WHERE client_slug='${slug}' ORDER BY observed_at DESC LIMIT 1`)[0];
  console.log("computed now      :", computed);
  console.log("latest in the log :", logged?.set_hash, `(${logged?.question_count} questions, ${logged?.at})`);
  console.log("MATCH             :", computed === logged?.set_hash);
}
main();
