...
async function main() {
  const slug = process.argv[2] ?? "hawaii-theatre";
  const cur: any = runSql(`SELECT body_markdown b FROM monthly_memos WHERE client_slug='${slug}' AND delivered_at IS NULL ORDER BY month_key DESC LIMIT 1`)[0];
  const pri: any = runSql(`SELECT body_markdown b FROM monthly_memos WHERE client_slug='${slug}' AND delivered_at IS NOT NULL ORDER BY month_key DESC LIMIT 1`)[0];
  if (!cur) { console.log("no draft"); return; }
  const prior = priorDeliveredNumbers(pri?.b);
  console.log(`${slug}: prior delivered memo ${pri ? "found, " + prior.size + " figures" : "NONE"}`);
  // Deliberately empty allowed-set: isolates ONLY the two exemptions.
  const before = findUnverifiedNumbers(cur.b, new Set(), new Set(), new Set());
  const after  = findUnverifiedNumbers(cur.b, new Set(), new Set(), prior);
  const fixed = before.filter((n: string) => !after.includes(n));
  console.log("no longer flagged by the two exemptions:", fixed.join(", ") || "(none)");
}
main();
