...
async function main() {
  ...
  const gridQ: string[] = f?.grid?.questions ?? [];
  const inpQ: string[] = inp.by_question.map((q: any) => q.keyword);
  const onlyGrid = gridQ.filter((q) => !inpQ.includes(q));
  const onlyInp = inpQ.filter((q) => !gridQ.includes(q));
  console.log(`${slug} ${month}`);
  console.log("  facts grid questions :", gridQ.length);
  console.log("  memo by_question     :", inpQ.length);
  console.log("  in grid only         :", onlyGrid.length, onlyGrid.slice(0,3).map(q=>q.slice(0,38)));
  console.log("  in memo only         :", onlyInp.length, onlyInp.slice(0,3).map(q=>q.slice(0,38)));
  console.log("  AGREE                :", onlyGrid.length === 0 && onlyInp.length === 0);
}
main();
