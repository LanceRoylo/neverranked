...
const CONCERN: Record<string, { severity: AlertSeverity; fix: string }> = {
  ...
  engine_quota_exhausted: { severity: "high", fix: "A surface refused a measurement call because the account balance or spend limit is exhausted. This never clears on its own and no retry reaches it: every remaining question on that surface is refused until someone pays. Add credit at the provider, then confirm with the live engine probe on /admin/health instead of waiting for the next 06:00 UTC sweep." },
  instrument_probe_failed: { severity: "high", fix: "The daily instrument probe got no answer from a surface. It is one tiny call per engine, so a failure means the API refused us: dead or rotated key, retired model id, exhausted credit balance, or a monthly spend / rate limit reached. Verify the key and the provider billing page." },
  anomaly_engine_peer_drop: { severity: "high", fix: "A surface produced far fewer rows than its sibling surfaces in the same 24h (all engines get the same questions on the same schedule). Unlike the rolling-baseline check this stays loud while the problem persists. Check the engine's API key, credit balance, monthly spend limit and rate limit. Any client readout covering these days will silently omit this engine." },
