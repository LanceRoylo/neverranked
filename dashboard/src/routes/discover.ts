...
export async function handleDiscoverList(slug: string, user: User, env: Env): Promise<Response> {
  ...
  const pendingHtml = pending.length === 0 ? `
    <div style="border:1px dashed var(--line);border-radius:6px;padding:32px 24px;text-align:center;color:var(--text-mute);font-size:13px">
      ...
      ${trackedCount === 0
        ? "Click <em>Generate suggestions</em> to get started. We'll read your business context and propose 15 realistic prompts your prospects might type into ChatGPT, Perplexity, Claude, Gemini, Microsoft Copilot, Google AI Overviews, and Gemma."
  ...
  const body = `
    <div style="margin-bottom:24px">
      ...
      <p style="color:var(--text-mute);max-width:680px;margin-top:8px">
        AI-suggested prompts your prospects might type into ChatGPT, Perplexity, Claude, Gemini, Microsoft Copilot, Google AI Overviews, and Gemma. Generated from your business context (description, schema, top organic queries). Review and one-click <strong style="color:var(--text)">Track</strong> the ones worth monitoring.
      </p>
    </div>
