...
export async function handlePreviewBuildForProspect(
  prospect_id: number, user: User, env: Env,
): Promise<Response> {
  if (user.role !== "admin") return redirect("/");

  // Don't re-generate if there's already an active Preview for this prospect.
  ...
  // Sonnet pass to write the body. For v1 we don't have prospect
  // metadata in D1 (names + emails live in the local outreach SQLite),
  // so the brief is category-neutral. Lance edits in the admin UI
  // before publishing.
  const generated = await generatePreview(env, { prospect_id });
  if (!generated) {
  ...
  const slug = await savePreviewDraft(env, { prospect_id }, generated);
