...
export async function getUser(request: Request, env: Env): Promise<User | null> {
  ...
  const row = await env.DB.prepare(
    `SELECT u.id, u.email, u.name, u.role, u.client_slug, u.onboarded, u.created_at, u.last_login_at
...
export async function verifyMagicLink(
  ...
  const user = await env.DB.prepare(
    "SELECT id, email, name, role, client_slug, onboarded, created_at, last_login_at FROM users WHERE email = ?"
