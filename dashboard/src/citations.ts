...
/** What an engine call returns.
 *
 * `failure` is the whole point. Until 2026-08-27 every HTTP error mapped to
 * {text:"", urls:[], entities:[]} -- an EMPTY SUCCESS -- and the skip-on-empty
 * guards then dropped it, so a 429 and "the engine had nothing to say" were
 * the same absence of a row. That guard was itself a fix: in July a dead
 * OpenAI account wrote 330 empty rows over nine days while the heartbeat
 * counted rows and called the engine healthy. Skipping empties cured the false
 * positive and created the mirror failure, an invisible one.
 *
 * Measured 2026-08-27, 48h: anthropic 121, bing 121, gemini 121, gemma 120,
 * google_ai_overview 100, perplexity 97, openai 39. Two calls in three were
 * failing and the only evidence was a row count, because the reason had been
 * thrown away at the point it was known.
 *
 * Carrying the reason changes "openai wrote fewer rows" into "openai got N
...
/** One place that decides whether an engine result is a measurement.
 *
 * Both branches skip persistence -- citation_runs must never hold a row that
...
async function skipReason(
  ...
  if (nothing) {
    console.log(`[engine-skip] ${engine} completed but returned nothing for "${keyword}" -- a real empty answer, not a failure`);
    return true;
  }
  return false;
}
...
async function queryPerplexity(
  ...
  if (!resp.ok) {
    ...
    console.log(`[engine-failure] perplexity ${resp.status} for "${keyword}": ${err.slice(0, 300)}`);
    return { text: "", urls: [], entities: [], failure: { engine: "perplexity", status: resp.status, detail: err.slice(0, 300) } };
  }
  ...
  if (data.status === "failed") {
    ...
    const pmsg = data.error?.message || "no error message";
    console.log(`[engine-failure] perplexity agent-failed (HTTP 200) for "${keyword}": ${pmsg}`);
...
async function queryOpenAI(
  ...
  if (!resp.ok) {
    const err = await resp.text();
    console.log(`[engine-failure] openai ${resp.status} for "${keyword}": ${err.slice(0, 300)}`);
    return { text: "", urls: [], entities: [], failure: { engine: "openai", status: resp.status, detail: err.slice(0, 300) } };
  }
...
async function queryGemini(
  ...
  try {
    ...
    console.log("[engine-failure] gemini non-JSON response: " + rawJson.slice(0, 300));
    return { text: "", urls: [], entities: [], failure: { engine: "gemini", status: 0, detail: "non-JSON response: " + rawJson.slice(0, 260) } };
  }
  ...
  for (const [clientSlug, kwList] of clientKeywords) {
    ...
    for (const kw of kwList) {
      ...
      const runPerplexity = async () => {
        ...
        ).bind(kw.id, r.text.slice(0, RESPONSE_TEXT_CAP), JSON.stringify(r.entities), JSON.stringify(r.urls), (r.citedStrict ? JSON.stringify(r.citedStrict) : null), JSON.stringify(r.retrievedUrls ?? r.urls), cited ? 1 : 0, prom, now).run();
      ...
      const runOpenAI = async () => {
        ...
        const insertRes = await env.DB.prepare(
          `INSERT INTO citation_runs (keyword_id, engine, response_text, cited_entities, cited_urls, cited_urls_strict, retrieved_urls, client_cited, prominence, run_at, grounding_mode)
           VALUES (?, 'openai', ?, ?, ?, ?, ?, ?, ?, ?, 'web')`
        ).bind(kw.id, r.text.slice(0, RESPONSE_TEXT_CAP), JSON.stringify(r.entities), JSON.stringify(r.urls), null, JSON.stringify(r.urls), cited ? 1 : 0, prom, now).run();
      ...
      const runAnthropic = async () => {
        ...
        ).bind(kw.id, r.text.slice(0, RESPONSE_TEXT_CAP), JSON.stringify(r.entities), '[]', '[]', cited ? 1 : 0, prom, now).run();
...
export async function runOneKeywordCitations(
  ...
  const runPerplexity = async () => {
    ...
    await env.DB.prepare(
      `INSERT INTO citation_runs (keyword_id, engine, response_text, cited_entities, cited_urls, cited_urls_strict, retrieved_urls, client_cited, prominence, run_at, grounding_mode)
       VALUES (?, 'perplexity', ?, ?, ?, ?, ?, ?, ?, ?, 'web')`
    ).bind(kw.id, r.text.slice(0, RESPONSE_TEXT_CAP), JSON.stringify(r.entities), JSON.stringify(r.urls), (r.citedStrict ? JSON.stringify(r.citedStrict) : null), JSON.stringify(r.retrievedUrls ?? r.urls), cited ? 1 : 0, prom, tick()).run();
  ...
  const runOpenAI = async () => {
    ...
    await env.DB.prepare(
      `INSERT INTO citation_runs (keyword_id, engine, response_text, cited_entities, cited_urls, cited_urls_strict, retrieved_urls, client_cited, prominence, run_at, grounding_mode)
       VALUES (?, 'openai', ?, ?, ?, ?, ?, ?, ?, ?, 'web')`
    ).bind(kw.id, r.text.slice(0, RESPONSE_TEXT_CAP), JSON.stringify(r.entities), JSON.stringify(r.urls), null, JSON.stringify(r.urls), cited ? 1 : 0, prom, tick()).run();
  ...
  const runAnthropic = async () => {
    ...
    await env.DB.prepare(
      `INSERT INTO citation_runs (keyword_id, engine, response_text, cited_entities, cited_urls, cited_urls_strict, retrieved_urls, client_cited, prominence, run_at, grounding_mode)
       VALUES (?, 'anthropic', ?, ?, '[]', ?, ?, ?, ?, ?, 'training')`
    ).bind(kw.id, r.text.slice(0, RESPONSE_TEXT_CAP), JSON.stringify(r.entities), '[]', '[]', cited ? 1 : 0, prom, tick()).run();
...
/**
 * Replicate sweep, split into a PLAN and a per-group RUN.
 *
 * WHY SPLIT. The first version did every call inline in the cron handler: up to
 * 81 sequential API calls at nine to sixteen seconds each. This codebase has
 * already paid for that shape once. From the fan-out note below:
 * runWeeklyCitations() inside a single step.do() "silently hit the per-step 30s
 * CPU ceiling after ~1.5 keywords and exited successfully with most data
 * missing". An audit caught the repeat before it ran.
 *
 * It would have been worse here than there. Partial groups are dropped by
 * design, so a truncated sweep produces fewer groups rather than broken ones,
 * the cron log records success, and the noise floor gets computed from a
 * silently biased sample. A guard reporting success while measuring nothing,
 * built by the person who spent the day removing them.
 *
 * So the workflow runs one step.do() per group, each with its own fresh budget,
 * exactly like the citation fan-out. Three readings still happen back to back
 * inside a single step, which is what makes them replicates.
 */
export interface ReplicatePlanItem {
  batchId: string;
  keywordId: number;
  keyword: string;
  engine: string;
...
export async function planReplicateSweep(env: Env, weekIndex: number): Promise<ReplicatePlanItem[]> {
  ...
  const ctx = new Map<string, { domain: string; name: string | null }>();
  const out: ReplicatePlanItem[] = [];
  for (const pick of chosen) {
    if (!ctx.has(pick.clientSlug)) {
      const domain = await env.DB.prepare(
        "SELECT * FROM domains WHERE client_slug = ? AND is_competitor = 0 AND active = 1 LIMIT 1",
      ).bind(pick.clientSlug).first<Domain>();
      const config = await env.DB.prepare(
        "SELECT * FROM injection_configs WHERE client_slug = ?",
      ).bind(pick.clientSlug).first<InjectionConfig>();
      ctx.set(pick.clientSlug, {
        domain: domain?.domain || "",
        name: await resolveBusinessName(env, pick.clientSlug, config),
      });
    }
    const c = ctx.get(pick.clientSlug)!;
    const kw = byId.get(pick.keywordId);
    if (!kw || !c.domain) continue;
    for (const engine of engines) {
      if ((out.length + 1) * REPLICATES > MAX_CALLS_PER_RUN) return out;
      out.push({ batchId, keywordId: kw.keywordId, keyword: kw.keyword, engine, clientDomain: c.domain, businessName: c.name });
    }
  }
  return out;
}
...
/**
 * One group: the same question asked REPLICATES times back to back on one
 * engine. Reuses computeProminence, skipReason and resolveBusinessName via the
 * plan rather than reimplementing them, so "cited" means here exactly what it
 * means in the production sweep.
 */
export async function runReplicateGroup(env: Env, item: ReplicatePlanItem): Promise<number> {
  const { REPLICATES } = await import("./lib/replicate-sample");
  const { recordSpend } = await import("./lib/engine-spend");

  const call = async (): Promise<EngineResult | null> => {
    try {
      if (item.engine === "perplexity" && env.PERPLEXITY_API_KEY) return await queryPerplexity(item.keyword, env.PERPLEXITY_API_KEY);
      if (item.engine === "openai" && env.OPENAI_API_KEY) return await queryOpenAI(item.keyword, env.OPENAI_API_KEY);
      if (item.engine === "gemini" && env.GEMINI_API_KEY) return await queryGemini(item.keyword, env.GEMINI_API_KEY, env);
    } catch { /* a lost reading drops the group; never counted as agreement */ }
    return null;
  };
  ...
  for (let rep = 0; rep < REPLICATES; rep++) {
    ...
    await recordSpend(env, item.engine, r.usage, item.keywordId, ts);
    if (await skipReason(env, item.engine, item.keyword, r, false)) continue;
    const prom = computeProminence(r.entities, r.urls, item.clientDomain, item.businessName);
    try {
      await env.DB.prepare(
        `INSERT OR IGNORE INTO replicate_runs
           (batch_id, keyword_id, engine, rep_index, run_at, client_cited, cited_urls)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).bind(item.batchId, item.keywordId, item.engine, rep, ts, prom !== null ? 1 : 0, JSON.stringify(r.urls)).run();
      wrote++;
    } catch (e) {
      console.log(`[replicate] insert failed ${item.engine}/${item.keywordId}: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  return wrote;
}
