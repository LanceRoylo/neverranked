...
function generatePhase2(ctx: ScanContext): RoadmapSeed[] {
  ...
  items.push({ title: "Publish one definitive guide on your core topic", description: "Comprehensive single-source pages appear among cited sources more often than scattered shorter posts. Pick one topic where you have deep expertise (the thing you'd write if you only got to write one piece) and write a 2,000+ word guide with proper headings, internal links, and a few citations. This becomes the page AI engines reach for first when answering about your area.", category: "content", priority: p++ });
...
function generatePhase3(ctx: ScanContext): RoadmapSeed[] {
  ...
  items.push({ title: "Build a network of interlinked content covering your topic", description: "Cited sources tend to demonstrate deep, structured expertise on a single topic over sources with one or two pages. Plan a topic cluster: one pillar page on the core topic, 5-10 supporting pages on subtopics, all interlinked. Over 6-12 months this becomes the source AI reaches for when summarizing your area.", category: "content", priority: p++ });

  items.push({ title: "Test your visibility across ChatGPT, Perplexity, Claude, Gemini, Google AI Overviews, and Gemma, read against Bing organic as a classic-search control", description: "The engines split into two kinds (Perplexity is web-grounded and shows links; Claude answers from training data and cites nothing at all, so it is scored on whether it NAMES you; ChatGPT's Search is closer to Perplexity's pattern; Gemma is open-weight, so the same prompt is independently reproducible). Test your top 10 buyer queries in each engine, note where you appear and where you don't, and prioritize the engines your buyers use most.", category: "authority", priority: p++ });
