# Ranking #1, traffic zero

A real story (anonymized). Use across LinkedIn, X (as a thread),
and IG (as a single-image post with shorter caption).

---

## LinkedIn (Lance, personal)

An old friend asked me to take a look at his small business this
week. He has been paying for a Google rank tracker. He sent me
his last 30 days.

Average position 15.1 across 26 keywords. Best in his category.
His two closest rivals rank at 26.2 and 28.8.

His estimated traffic. Zero.

Theirs. 44 and 72.

Worse rank, more traffic. He is winning the leaderboard and
losing the customer.

Here is what is happening. Apple Intelligence is on by default
in Safari on every recent iPhone. Google AI Overviews sit above
the blue links for the queries his buyers actually search.
ChatGPT and Perplexity answer the question before anyone reaches
a results page.

The rank does not matter if the click never happens.

His tracker cannot show him this, because his tracker watches
Google's blue-link results. The thing taking his customers is
happening one layer up, in answers his tool was not built to see.

If your traffic is dropping while your Google rankings hold or
improve, that is not a mystery. That is the AI search shift on
your invoice.

Anyone else seeing this in their data?

(Free schema scorer at check.neverranked.com if you want to see
where you stand.)


## LinkedIn (Never Ranked company page version)

Same story as Lance's personal post above, framed for the company
page. Less first-person, more "we are seeing this pattern across
our customer base." Use the personal version first. If the post
performs, reshape and run from the company page two weeks later.

---

## X / Threads (single post or threadable)

A small business owner sent me his rank tracker.

Average position 15.1. Best in his category. His two closest
rivals rank 26.2 and 28.8.

His estimated traffic: zero.
Their estimated traffic: 44 and 72.

He is winning the leaderboard and losing the customer.

Apple Intelligence and AI Overviews intercept the click before
the rank matters. The fix is structured data the AI engines can
read, not a better rank tracker.

check.neverranked.com if you want to see your own gap.

---

## Instagram (Never Ranked company page)

A small business we recently reviewed ranks number one in their
category on Google.

Their estimated traffic. Zero.

Their two closest rivals rank at 26.2 and 28.8. Their traffic.
44 and 72 visits.

Worse rank, more traffic. They are winning the leaderboard and
losing the customer.

Apple Intelligence is on by default in iPhone Safari. Google AI
Overviews sit above the blue links for the queries buyers
actually search. ChatGPT and Perplexity answer the question
before anyone reaches a results page.

The rank does not matter when the click never happens.

If your traffic is dropping while your rankings hold or improve,
that is not a mystery. That is the AI search shift on your
invoice.

Free six-engine scan at check.neverranked.com. Link in bio.

#AEO #AnswerEngineOptimization #AISearch #SEO2026 #SchemaMarkup
#SmallBusinessSEO #AIcitations #NeverRanked

---

## Instagram (Lance, personal — alternate)

If posting from Lance's personal Instagram instead of Never
Ranked's company page, swap the opener:

> "An old friend asked me to take a look at his small business
> this week. His Google rank tracker says he ranks number one
> in his category. His estimated traffic. Zero."

Rest of the caption stays the same. Tighter and more personal,
matching the LinkedIn personal voice above.
